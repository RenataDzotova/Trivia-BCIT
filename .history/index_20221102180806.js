...
app.get('/true', (req, res)=>{
  // console.log(req.query) //? This will now make an object out of whatever query is inserted in the searchbar, for example, "localhost:3000/?name=cocoa" will console.log { name: 'cocoa' }
  res.json("Unfortunately that fact was true")
})

app.get('/false', (req, res)=>{
  // console.log(req.query) //? This will now make an object out of whatever query is inserted in the searchbar, for example, "localhost:3000/?name=cocoa" will console.log { name: 'cocoa' }
  res.json("Correct, that fact was false!")
})
...
let body = req.body //body key is for posts
//params is for gets
console.log('HERE', body, req.body, req.query)
if(Number(req.query.answer) === answerkey[req.query.quest]){
  res.json(true);
} else {
  res.json(false);
  console.log(req.query.answer)
  console.log(answerkey[req.query.answer])
}
